...
=item newBeaconTmpFile

Path to new Beacon List indicator file pinger2 waits before acting on a new update interval.
...
#my $version="2.0.4, 2/5/2012 by Les Cottrell";#Verify lynx returns something
#my $version="2.0.5, 2/19/2012 by Fahad Satti & Les Cottrell.";#Update Beacon List, if it was requested via ping_data.pl
#my $version="2.0.6, 11/12/2012 by Les Cottrell";#Allow for icmp-req
#my $version="2.0.7, 3/8/2014 by Les Cottrell & Raja Asad";#Diagnose 0 length pinger.xml & full directory
my $version="2.0.7, 3/18/2014 by Les Cottrell";#Improve check for SrcIP
...
sub checkBeaconConfiguration()
{
	...
	if (time - $lastChecked > $refreshInterval)
	{
	    ...
	    # Entries that are not to be deleted have to go into <HostList>
            # Next section modified by Cottrell 2/3/2012
            unless(scalar(@beacons)<=0) {
              $beaconList = $config->{'BeaconList'}->{'Host'} = [];
              &logger("<BeaconList> populated with "
                     . scalar(@beacons)." lines using $httpGetBin", 4);
            }
            else {
	      &logger(`date`.": empty <BeaconList>(".scalar(@beacons).") "
                    . "returned by $httpGetBin(@which).",0);
            }             
	    my $line; my $nline;   
		...
		push(@{$beaconList}, $host);
            ...
            #Following section mod by Cottrell 2/4/2012.
            #unless($nline<=0) {#If lines inserted into <BeaconList> update the checked time
            if(defined($nline) && $nline>0) {#If lines inserted into <BeaconList> update the checked time
	      $beaconConfig->{'LastChecked'} = time;
              &logger("BeaconList updated. #lines read=".scalar(@beacons)." lines inserted=$nline\n", 0);
            }
            else {#Else don't update checked time so will try again next time pinger2.pl is run
              &logger("BeaconList not updated. Found $nline valid lines in "
                      .scalar(@beacons)." lines returned by $httpGetBin(@which): "
                      ."@beacons\n", 0);
            }
	} else {
	    &logger("BeaconList is still up to date. "
	           . time . " - \$lastChecked($lastChecked) > "
...
sub _getIP($$){
    my $ipv4 = $_[0];
    my $hostname = $_[1];
    #Mod by Cottrell 2/5/2012.
    unless(($hostname=~/(([a-z0-9]+|([a-z0-9]+[-]+[a-z0-9]+))[.])+/)){
      &logger("_getIP(addr=$ipv4, $hostname): hostname='$hostname' invalid IP name\n",3);
      return undef;
    }
    my $dnsCmd;
    my $msg= scalar(localtime())." local time "
            ."processing $hostname";
    if ($ipv4){
	&logger("getIPV4: $msg\n", 7);
	$dnsCmd = $dnsV4Cmd;
    ...
    &logger("_getIP($ipv4,$hostname): dnsCmd=$dnsCmd, destination=$hostname\n",9);
...
sub doPing($$$$$$)
{
    ...
    my $temp='undef';
    if(defined($resultsMatter)) {$temp=$resultsMatter;};
    &logger("doPing($ip,$interval,$size,$numPings,$dest,$temp): "
          . "Expanded pingCmd resolved to '$pingCmd'\n", 5);
    my $pid = open(PING, "-|");
    if ($pid == 0) {
...
sub log_it {
	...
	if (!(sysopen(PINGDATA, $ping_data_fn, (O_RDWR | O_EXCL | O_CREAT)))) {
		...
		print STDERR scalar(localtime())
                    . " Can't open PINGDATA $ping_data_fn: $!\n";
...
sub pingHost($){
    ...
    &logger("pingHost(host=$name): Enter.\n", 9);
    if($name eq "") {
      &logger("pingHost(name=$name): ignored null host\n",3);
      return;
    }
    ...
    $name = &getHostAttribute($host, 'Name');
...
sub pingAllHosts(){
    ...
    logger("pingAllHosts(): Starting to ping all ".scalar(@{$beaconList})." beacons.\n",3);
...
Writes out XML information to the selected file.

Input: the file name to be written to
...
sub doXMLOut(){
  my $file=$_[0];
  my $string = XMLout($config, RootName => 'Pinger', noattr => 1);
  if(!defined($string) || $string eq ''){print STDERR "XMLout returned empty str
	    ing, now exiting\n";exit 0}
  open(PINGERXML,">$file") || die "Can't open $file for writing, now exiting: $!";
  print PINGERXML $string or die "Can't write to $file".$!;
  close(PINGERXML) || die "Can't close $file".$!;
  logger("doXMLout(): wrote XML file to $file\n", 4);
 }
########################################################
# Main Program
########################################################
...
$logLevel = 10;
$logFile = *STDOUT;
#Make sure we have a pinger.xml configuration file
my $xmlfile='pinger.xml';
if(-z '$xmlfile') {
  print STDERR `df`;
  print STDERR `ls -l $xmlfile`; 
  print STDERR `pwd`;
  die scalar(localtime())." local time: $xmlfile has zero size at ";
}
# Load the Pinger configuration and use Name as a key into the hostlist
#system("cat $xmlfile");
$config = XMLin("$xmlfile", ForceArray => ['Ping', 'Host']);
&logger(scalar(localtime()). " $0(version=$version) "
      . "loaded $xmlfile config for "
      . scalar(getpwuid($<))."\@$config->{'SrcName'}\n",7);
$beaconList    = &getList('BeaconList');
$hostList      = &getList('HostList');
$defaultHost   = $config->{'DefaultHost'};
$dataDirectory = $config->{'DataDirectory'};
&logger("\$dataDirectory=$dataDirectory, now getting IP address\n",9);    
$pingV4Cmd = $config->{'PingV4Cmd'};
if ($pingV4Cmd =~ /NOT_SUPPORTED/){
  print "Unable to get \$config->'{'PingV4Cmd} from $xmlfile\n";
  system("cat $xmlfile");
...
if ((not defined $pingV6Cmd) || $pingV6Cmd =~ /NOT_SUPPORTED/){
    print STDERR "PingV6Cmd not supported. IPV6 entries cannot be pinged!\n";
    system('pwd');
    system("cat $xmlfile");
    undef $pingV6Cmd;
}
...
if (not defined($srcIP)){
    ...
    if (not defined($srcIP)){
	die "SrcIP could not be determined for $srcName. Without the results produced by PingER2 are not useable.\n";
    }
}
logger("Src=$srcName($srcIP)\n", 9);
...
##############################################################################
#Check if there exists a file in /tmp that indicates to fetch a new beacon list.
#This file is created by ping_data.pl to indicate that we want to renew the
#<BeaconList> file in pinger.xml.  If it does exist then the date the
#pinger.xml was last updated is set to 0 (Jan 01, 1970) so the next
#time pinger2.pl runs it will update pinger.xml with the new
#<BeaconList>. At the same time as setting the last update time to 0
#the tmp file is deleted. Note this idea only works if the host on which
#the tmp file is created (e.g. by ping_data.pl running in a web server)
#is the same host that pinger2.pl is executed from by a crontab.
#This section added by Fahad and Les Feb 18, 2011
my $newBeaconTmpFile = "/tmp/pinger2_newBeaconList.txt";
if(-e $newBeaconTmpFile){
    &logger("Remote request for a new Beacon List\n",3);
    $config->{'BeaconListConfig'}->{'LastChecked'}=0;
    &logger("Set last checked time to 0 will update <BeaconList> next time\n");
    &logger("Removing Temp File $newBeaconTmpFile\n"
          . `rm $newBeaconTmpFile`,3);
}
else {&logger("$newBeaconTmpFile does not exist on ".`hostname`, 3);}
#End of section added by Fahad and Les
################################################################
...
#&testGetIP();
use Data::Dumper;
#open(TMPFILE,">tmpfile.txt") or die "cant open tmp file";
##print TMPFILE Dumper($config);
# Save before we start pinging, in case the user breaks the process...
if(-z "$xmlfile") {
  print STDERR `ls -l $xmlfile`;
  print STDERR " $xmlfile has zero size at line ".__LINE__." before XMLout\n";
} 
#my $testvalue=XMLout($config, OutputFile => 'pinger.xml', RootName => 'Pinger', noattr => 1);
#print "testvalue is $testvalue\n\n";
&doXMLOut($xmlfile);
if(-z "$xmlfile") {
  print STDERR `ls -l $xmlfile`;
  print STDERR " $xmlfile has zero size at line ".__LINE__." following XMLout\n";   
}          
